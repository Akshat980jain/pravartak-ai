...
const TrackApplication = () => {
  ...
  const [recent, setRecent] = useState<Array<{ id: number; tracking_id: string; status: string; created_at: string }>>([]);
  ...
  const mockApplicationData = {
    ...
    amount: "₹ 50,000",
    timeline: [
      { stage: "Application Submitted", date: "2025-01-10 14:30", completed: true, description: "Application received successfully" },
      { stage: "Document Verification", date: "2025-01-12 10:15", completed: true, description: "All documents verified" },
      { stage: "Under Review by District Officer", date: "2025-01-14 09:00", completed: true, description: "Application under review" },
      { stage: "Approval Pending", date: "In Progress", completed: false, description: "Awaiting final approval" },
      { stage: "Payment Processing", date: "Pending", completed: false, description: "Will be processed after approval" },
      { stage: "Amount Credited", date: "Pending", completed: false, description: "DBT to bank account" }
    ]
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.status === 404) {
        setError("No application found for this ID. Try a sample below or use demo view.");
        setShowStatus(false);
      } else if (!res.ok) {
        ...
        setServerData({
          ...
          status: data.status === 'submitted' ? 'Submitted' : data.status.replace('_', ' '),
          amount: form.amount ? `₹ ${Number(form.amount).toLocaleString('en-IN')}` : "-",
          timeline: [
            { stage: "Application Submitted", date: data.created_at, completed: true, description: "Application received successfully" },
            { stage: "Under Review", date: data.updated_at, completed: data.status !== 'submitted', description: "Application under review" },
          ],
        });
        setShowStatus(true);
      }
          ...
          <Card className="mb-8 border-primary/30">
            ...
            <CardContent className="pt-6">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <Button variant="outline" size="sm" onClick={useDemo}>Use demo view</Button>
                {recent.length > 0 && <span className="text-sm text-muted-foreground">Try sample IDs:</span>}
                {recent.map((r) => (
                  <Button key={r.id} variant="secondary" size="sm" onClick={() => { setApplicationId(r.tracking_id); }}>
                    {r.tracking_id}
                  </Button>
                ))}
              </div>
                    ...
                    {(serverData ?? mockApplicationData).timeline.map((stage: any, index: number) => (
                      <div key={index} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          ...
                          {index < mockApplicationData.timeline.length - 1 && (
                            <div className={`w-0.5 h-16 ${stage.completed ? 'bg-green-500' : 'bg-gray-200'}`} />
                          )}
                        </div>
                        <div className="flex-1 pb-6">
                          <div className={`p-4 rounded-lg border ${stage.completed ? 'bg-green-50 border-green-200' : stage.date === 'In Progress' ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <p className="text-sm font-semibold">
                              {stage.date === "Pending" ? "⏳ Awaiting previous step completion" : 
                               stage.date === "In Progress" ? "🔄 Currently in progress" : 
