...
router.get('/track/:trackingId', (req, res) => {
    const resDb = getDb().exec(`SELECT id, tracking_id, status, created_at, updated_at, scheme_id FROM applications WHERE tracking_id = '${req.params.trackingId.replace(/'/g, "''")}'`);
    const row = resDb[0]?.values?.[0];
    if (!row)
        return res.status(404).json({ error: 'Application not found' });
    res.json({ id: row[0], tracking_id: row[1], status: row[2], created_at: row[3], updated_at: row[4], scheme_id: row[5] });
});
router.get('/', (_req, res) => {
    const resDb = getDb().exec('SELECT id, tracking_id, status, created_at, updated_at, scheme_id FROM applications ORDER BY id DESC');
    const rows = (resDb[0]?.values ?? []).map((r) => ({ id: r[0], tracking_id: r[1], status: r[2], created_at: r[3], updated_at: r[4], scheme_id: r[5] }));
    res.json(rows);
});
