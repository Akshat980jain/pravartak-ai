...
router.get('/', (_req, res) => {
  ...
  const rows = (result[0]?.values ?? []).map((r: any[]) => ({ id: r[0] as number, title: r[1] as string, description: r[2] as string, department: r[3] as string, created_at: r[4] as string }));
  res.json(rows);
});
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    const merged = {
      ...
      department: (body as any).department ?? existing[3],
    };
...
router.delete('/:id', (req, res) => {
  ...
  if (before === 0 || after === before) return res.status(404).json({ error: 'Scheme not found' });
  persist();
  res.status(204).send();
});
