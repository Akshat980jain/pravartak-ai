...
export async function ensureDatabase(): Promise<void> {
  if (db) return;
  const SQLModule = await initSqlJs({ locateFile: (file: string) => `node_modules/sql.js/dist/${file}` });
  let buffer: Uint8Array | undefined = undefined;
  if (fs.existsSync(ENV.DB_FILE)) {
    buffer = fs.readFileSync(ENV.DB_FILE);
  }
  const database: Database = buffer ? new SQLModule.Database(buffer) : new SQLModule.Database();
...
function seedIfEmpty(database: Database): void {
  const [{ c }] = database.exec('SELECT COUNT(*) as c FROM schemes')[0]?.values.map((row: any[]) => ({ c: Number(row[0]) })) ?? [ { c: 0 } ];
  if (c === 0) {
